    ...
    def __init__(self, params):
        ...
        if self.expl_type.startswith('mixed_expl'):
            ...
            if 'disjoint' in self.expl_type or 'learn_param' in self.expl_type:
                ids = torch.linspace(50.0, 0.0, 64).to(self.device_name).reshape(-1,1)
            else:
                ids = create_sinusoidal_encoding(torch.linspace(50.0, 0.0, 64), self.config.get('expl_reward_coef_embd_size', 32), n=100).to(self.device_name)
