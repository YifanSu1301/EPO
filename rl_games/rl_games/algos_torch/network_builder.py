    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            if self.net_type == 'extra_param':
                ...
                print('self.param_ids:', self.param_ids)
                print('self.pid_idx:', self.pid_idx)
        ...
        def merge_extra_params(self, merged_policies, replaced_policy):
            # Ensure that merged_policies contains exactly two policies
            assert len(merged_policies) == 2, "merged_policies should contain exactly two policies"
            print('parmas of replaced_policy:', self.extra_params[replaced_policy])
            print('parmas of merged_policies[0]:', self.extra_params[merged_policies[0]])
            print('parmas of merged_policies[1]:', self.extra_params[merged_policies[1]])
            ...
            # Replace the extra_params of the replaced_policy with the merged_params
            ...
            print('merged_params:', self.extra_params[replaced_policy])
            # assert False
            return

        def cluster_extra_params(self):
            """
            Clustering the extra_params using K-Means to group into 8 clusters.
            Returns the number of clusters and the cluster labels.
            """
            from sklearn.cluster import KMeans
            from sklearn.preprocessing import StandardScaler

            # Get the extra_params
            extra_params = self.extra_params.detach().cpu().clone().numpy()

            # Ensure extra_params is 2D
            extra_params = extra_params.reshape(-1, 1) if extra_params.ndim == 1 else extra_params

            # Standardize the data (optional)
            extra_params = StandardScaler().fit_transform(extra_params)

            # Apply K-Means clustering with 8 clusters
            kmeans = KMeans(n_clusters=8, random_state=42, n_init=10)
            kmeans.fit(extra_params)

            # Get the cluster labels
            labels = kmeans.labels_

            # Return the number of clusters and the labels
            return 8, labels  # Always 8 clusters as specified 

        def vis_cluster_extra_params(self):
            # using t-SNE to visualize the clustering of the extra_params
            # Get the extra_params
            extra_params = self.extra_params.detach().cpu().clone().numpy()
            ...
            # Apply t-SNE to reduce the dimensionality of the extra_params to 2D
            # t-SNE is used for visualization purposes only
            # It is not used for clustering
            from sklearn.manifold import TSNE
            tsne = TSNE(random_state = 0, n_iter = 1000, metric = 'cosine')
            extra_params_2d = tsne.fit_transform(extra_params)

            # Return the 2D representation of the extra_params
            return extra_params_2d 
            
        
        def eliminate_extra_params(self, best_policy, eliminated_policy):
            extra_params_copy = self.extra_params.detach().clone()
            extra_params_copy[eliminated_policy] = extra_params_copy[best_policy]
